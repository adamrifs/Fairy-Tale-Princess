...
import { SECTION2_ID, SECTION2_SCENE_ID } from "@/data/story/section2";

/**
 * Background renderer for Section 2 — a single continuous frame sequence
 * (public/sections/section-2/, ~300 frames), driven 1:1 by scroll.
 *
 * Uses the exact same optimized loading path as Section 1: the full
 * compressed blob set is preloaded up front by the MemoryManager/
 * FrameManager pipeline (keyed by sectionId/sceneId), and this component's
 * useWindowedFrames slides a decode window across those already-local
 * blobs so scrubbing never waits on the network. The generic
 * FrameSequencePlayer canvas draws whatever frame it's handed.
 *
 * Unlike Section 1, there is no landing-video crossfade and no end video —
 * Section 2 opens directly on its frame sequence. A static frame_0001
 ...
 */
export const Section2Scene = memo(function Section2Scene({ progress = 0 }) {
  const { count, getDisplayFrame, loadWindow } = useWindowedFrames(SECTION2_ID, SECTION2_SCENE_ID);
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ backgroundImage: "url(/sections/section-2/frame_0001.webp)" }}
      />

      {/* ── Frame sequence ────────────────────────────────────────────────── */}
      <FrameSequencePlayer image={image} fallbackSrc="/sections/section-2/frame_0001.webp" />
