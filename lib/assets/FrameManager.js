...
class FrameSequence {
  ...
  loadFrames({ onProgress } = {}) {
    ...
    // Sequences past this size are too large to bulk-preload entirely
    // without causing an OOM or burning bandwidth. A small fixed preload
    // keeps the loading-bar experience while leaving the first seconds of
    // bandwidth to useWindowedFrames' ladder prefetch — on a real network
    // that ladder is what makes the first scroll smooth, and every frame
    // bulk-loaded here delays it (60 frames ≈ 6MB ≈ 2s at 25Mbps was
    // measurably starving it).
