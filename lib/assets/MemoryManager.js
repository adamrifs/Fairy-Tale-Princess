...
class MemoryManager {
  ...
  setActiveSection(sectionId, { onProgress } = {}) {
    ...
    // Warm the next section only AFTER the current one has fully
    // downloaded. Now that a section preloads its ENTIRE frame sequence
    // (~60MB), starting the next section's download concurrently would
    // split bandwidth and roughly double the current section's time to
    // "ready" — which on the very first section is exactly the loading
    // screen the user is staring at. Deferring keeps the critical path
    // uncontended, then fills the next section in during idle time.
    if (nextSectionId && !this.residentSectionIds.has(nextSectionId)) {
      currentLoad.then(() => {
        if (!this.residentSectionIds.has(nextSectionId)) {
